 ...
 * file. If equal then '##name' will be returned, otherwise exit code 2.
 *
...
int main(int argc, char **argv) {
	...
	if (salt[0] == '#' && salt[1] == '#') {
		...
		if (*pw->pw_passwd == 0 && *key == 0) {
			/* Fine */
		} else if (strcmp(crypt(key, pw->pw_passwd), pw->pw_passwd) != 0) {
			return 2;
		}
	} else {
		/* Normal encryption. */
	...
	if (write(STDOUT_FILENO, salt, strlen(salt) + 1) < 0)
	  return 1;
	return 0;
}
